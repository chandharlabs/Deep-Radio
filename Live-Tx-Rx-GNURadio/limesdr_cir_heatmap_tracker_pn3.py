...
def generate_pn_sequence(length=1023):
    state = 0b1111111
    taps = [7, 1]
    seq = []
...
pn_upsampled = pn_seq #np.repeat(pn_seq, 1)
tx_burst = np.concatenate([pn_upsampled, np.zeros(10000)])
...
class CIRAnalyzer(QMainWindow):
    ...
    def init_sdr(self):
        ...
        self.sdr.setAntenna(SOAPY_SDR_TX, 0, "BAND2")  # TX1_2
        ...
        self.sdr.setAntenna(SOAPY_SDR_RX, 0, "LNAH")  # RX1_H
