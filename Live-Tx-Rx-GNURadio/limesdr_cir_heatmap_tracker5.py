...
class CIRAnalyzer(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Live CIR Analyzer with PDP & Multipath Detection")
    ...
    def update_plot(self):
        ...
        # ----------- PLOT 3: Instantaneous PDP with Multipath Markers -----------
        self.ax3.clear()
        time_axis = np.arange(len(pdp)) / SAMPLE_RATE * 1e6
        pdp_db = 10 * np.log10(pdp + 1e-12)

        # Detect multiple paths above threshold (e.g., -6 dB)
        threshold_db = np.max(pdp_db) - 6
        strong_path_indices = np.where(pdp_db >= threshold_db)[0]

        self.ax3.plot(time_axis, pdp_db, label="PDP", color='green')
        self.ax3.scatter(time_axis[strong_path_indices],
                         pdp_db[strong_path_indices],
                         color='red', s=60, label='Strong Paths')

        # Annotate the strongest path
