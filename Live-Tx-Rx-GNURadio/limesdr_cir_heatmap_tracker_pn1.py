...
# ===== CONFIG =====
SAMPLE_RATE = 60e6            # 60 MSps
#NUM_SAMPLES = 16000
MAX_FRAMES = 50

# ===== PN SEQUENCE TX SIGNAL =====
def generate_pn_sequence(length=127):
    state = 0b1111111  # 7-bit LFSR
    taps = [7, 1]
    seq = []
...
tx_signal = np.concatenate([pn_upsampled, np.zeros(1000)])
# Automatically adapt RX length
TX_LEN = len(tx_signal)  # Length of transmitted signal
RX_MARGIN = 1000         # Additional samples to capture multipath

NUM_SAMPLES = TX_LEN + RX_MARGIN

#MAX_DELAY_US = 10  # Capture 10 microseconds of delay
#RX_MARGIN = int((MAX_DELAY_US * 1e-6) * SAMPLE_RATE)
#NUM_SAMPLES = TX_LEN + RX_MARGIN


print(f"[INFO] Auto-set NUM_SAMPLES = {NUM_SAMPLES}")

# ===== GUI APP =====
class CIRAnalyzer(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Real-Time CIR + Normalized PDP (LimeSDR)")
    ...
    def update_plot(self):
        ...
        cir_mag = np.abs(cir)
        pdp = cir_mag ** 2
        pdp /= np.sum(pdp) + 1e-12  # ✅ Normalize PDP

        # Save CIR for heatmap
        ...
        self.ax3.plot(time_axis, pdp_db, label="Normalized PDP", color='green')
        ...
        self.ax3.text(time_axis[max_idx], max_val + 2,
                      f"{time_axis[max_idx]:.2f} µs", color='red', fontsize=9)

        self.ax3.set_title("Normalized Power Delay Profile (PDP)")
        self.ax3.set_xlabel("Delay (µs)")
        self.ax3.set_ylabel("Power (dB)")
        self.ax3.set_ylim(-60, 0)
        self.ax3.grid(True)
        self.ax3.legend()
