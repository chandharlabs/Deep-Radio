...
# Generate 5G-like QPSK pilot
def generate_qpsk_pilot(length, seed=42):
    np.random.seed(seed)
    bits = np.random.randint(0, 2, 2 * length)
    qpsk = (1 - 2 * bits[0::2]) + 1j * (1 - 2 * bits[1::2])  # Gray mapping
    return (qpsk / np.sqrt(2)).astype(np.complex64)  # Normalize power

pilot_qpsk_tx1 = generate_qpsk_pilot(pilot_len, seed=1)
pilot_qpsk_tx2 = generate_qpsk_pilot(pilot_len, seed=2)

# Build full-frame signals
pilot_tx1 = np.zeros(num_samples, dtype=np.complex64)
pilot_tx2 = np.zeros(num_samples, dtype=np.complex64)
pilot_tx1[300:300 + pilot_len] = pilot_qpsk_tx1  # TX1_2
pilot_tx2[800:800 + pilot_len] = pilot_qpsk_tx2  # TX2_1
...
h11 = estimate_csi(rx1, pilot_qpsk_tx1)
h12 = estimate_csi(rx1, pilot_qpsk_tx2)
h21 = estimate_csi(rx2, pilot_qpsk_tx1)
h22 = estimate_csi(rx2, pilot_qpsk_tx2)

# Extract peak CSI values (strongest tap)
def extract_csi(h):
...
# Print complex CSI
print("Estimated CSI values (QPSK pilot):")
