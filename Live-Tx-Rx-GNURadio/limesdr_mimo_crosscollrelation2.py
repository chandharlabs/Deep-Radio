...
# TX Setup
sdr.setAntenna(SOAPY_SDR_TX, 0, "BAND1")  # TX1_2
...
# ==== Plot Setup ====
plt.ion()
fig, (ax_delay, ax_rx1, ax_rx2) = plt.subplots(3, 1, figsize=(12, 8))
...
# Rx1 and Rx2 signal plots
line_rx1, = ax_rx1.plot([], [], label="Rx1 |mag|", alpha=0.7)
ax_rx1.set_ylabel("Magnitude Rx1")
ax_rx1.set_xlabel("Sample Index")
ax_rx1.grid(True)
ax_rx1.legend()

line_rx2, = ax_rx2.plot([], [], label="Rx2 |mag|", alpha=0.7)
ax_rx2.set_ylabel("Magnitude Rx2")
ax_rx2.set_xlabel("Sample Index")
ax_rx2.grid(True)
ax_rx2.legend()
...
# ==== Main Loop Thread ====
def processing_loop():
    global frame, running, stop_requested, total_phase
    while True:
        if not running:
            time.sleep(0.1)
            continue
        ...
        if recording:
            ...
            np.savez(os.path.join(save_dir, f"frame_{frame}_{timestamp}.npz"),
                     rx1=buff1.copy(), rx2=buff2.copy(),
                     delay=filtered_lag, aoa=aoa_deg)
        ...
        if frame % plot_every_n == 0:
            ...
            n_disp = 1024
            mag1 = np.abs(buff1[:n_disp])
            mag2 = np.abs(buff2[:n_disp])
            line_rx1.set_data(np.arange(n_disp), mag1)
            line_rx2.set_data(np.arange(n_disp), mag2)
            ax_rx1.set_xlim(0, n_disp)
            ax_rx1.set_ylim(0, np.max(mag1) * 1.1)
            ax_rx2.set_xlim(0, n_disp)
            ax_rx2.set_ylim(0, np.max(mag2) * 1.1)
...
def toggle_run():
    global running
    running = not running
    btn_start_pause.config(text="Pause" if running else "Start")

def toggle_record():
    global recording
    recording = not recording
    btn_record.config(text="Stop Recording" if recording else "Start Recording")
...
gui = tk.Tk()
gui.title("LimeSDR AoA GUI")

btn_start_pause = ttk.Button(gui, text="Start", command=toggle_run)
btn_start_pause.pack(padx=20, pady=10)

btn_record = ttk.Button(gui, text="Start Recording", command=toggle_record)
btn_record.pack(padx=20, pady=10)

btn_stop = ttk.Button(gui, text="Stop", command=stop_app)
