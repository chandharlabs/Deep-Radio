...
rx1a = np.zeros(num_samples*2, dtype=np.complex64)
rx2a = np.zeros(num_samples*2, dtype=np.complex64)
sdr.readStream(rx_stream, [rx1a, rx2a], num_samples*2)

# --- Transmit Pilot from TX2_1 only ---
print("Transmitting from TX2_1 only...")
sdr.writeStream(tx_stream, [silent, pilot], num_samples)
time.sleep(0.1)
rx1b = np.zeros(num_samples*2, dtype=np.complex64)
rx2b = np.zeros(num_samples*2, dtype=np.complex64)
sdr.readStream(rx_stream, [rx1b, rx2b], num_samples*2)
...
plt.subplot(3, 2, 1)
plt.title("h11: TX1_2 → RX1_H (Magnitude)")
plt.plot(np.abs(h11))
plt.grid()

plt.subplot(3, 2, 2)
plt.title("h12: TX2_1 → RX1_H (Magnitude)")
plt.plot(np.abs(h12))
plt.grid()

plt.subplot(3, 2, 3)
plt.title("h21: TX1_2 → RX2_H (Magnitude)")
plt.plot(np.abs(h21))
plt.grid()

plt.subplot(3, 2, 4)
plt.title("h22: TX2_1 → RX2_H (Magnitude)")
plt.plot(np.abs(h22))
plt.grid()

plt.subplot(3, 2, 5)
plt.title("Raw Signal @ RX1_H (Real)")
plt.plot(np.real(rx1a), label="From TX1_2")
plt.plot(np.real(rx1b), label="From TX2_1", linestyle='--')
plt.legend()
plt.grid()

plt.subplot(3, 2, 6)
plt.title("Raw Signal @ RX2_H (Real)")
plt.plot(np.real(rx2a), label="From TX1_2")
plt.plot(np.real(rx2b), label="From TX2_1", linestyle='--')
plt.legend()
plt.grid()
