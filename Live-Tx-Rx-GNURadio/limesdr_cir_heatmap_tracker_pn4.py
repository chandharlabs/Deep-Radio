...
tx_burst = np.concatenate([pn_upsampled, np.zeros(1000)])
...
class CIRAnalyzer(QMainWindow):
    ...
    def compute_cir(self, rx):
        pn_ref = pn_upsampled  # correlate only with the PN
        corr = np.correlate(rx, pn_ref, mode='valid')
        corr = corr / (np.linalg.norm(pn_ref)**2 + 1e-12)
        start_idx = np.argmax(np.abs(corr))
        cir = corr[start_idx:start_idx + len(pn_ref)]
