...
class blk(gr.sync_block):
    def __init__(self):
        gr.sync_block.__init__(
            self,
            name='Burst Capture with Downsampling and ASCII Decode',
            in_sig=[np.uint8],  # Use np.float32 if input is float
            out_sig=None
        )

        self.zero_count = 0
        self.zero_threshold = 1000  # Long silence before/after bursts
    ...
    def _save_and_decode(self):
        ...
        #print(f"Captured {len(downsampled)} symbols → ASCII: {ascii_str.strip()}")
