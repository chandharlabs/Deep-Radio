...
#     iq_samples = read_samples(filename)
#    iq_samples = read_samples_fc32(filename)
    decimation_rate=1
...
###########################################################Test code############################
test_path = directory+'/'+testing_data_folder  # Folder for the data used for testing
print(test_path)  

model=pickle.load(open('model_nb.sav', 'rb'))
 
data = read_train_sets2(test_path, class_array, validation_size=0) # Gets the data object using a class in dataset2.py
...
print(confusion_matrix(Ypred1,Ytest1))
