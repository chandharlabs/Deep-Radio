...
def plot_iq_samples(samples, title):
...
def plot_spectrum(samples, sample_rate, title):
    plt.figure(figsize=(10, 6))
    plt.specgram(samples, NFFT=1024, Fs=sample_rate, noverlap=512)
    plt.title(title)
    plt.xlabel('Time [s]')
...
def main():
    ...
    # Plot the IQ samples
    plot_iq_samples(samples, 'IQ Samples')
    
    # Plot the spectrum of the IQ samples
    plot_spectrum(samples, sdr.sample_rate, 'Spectrum of IQ Samples')
    ...
    # Plot the demodulated audio signal spectrum
    plot_spectrum(audio_signal, audio_rate, 'Demodulated Audio Signal Spectrum')
