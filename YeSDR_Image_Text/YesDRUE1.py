...
def pc2_full_stack_rx(timeout_after_first=5.0):
    print("\n=== Starting Receiver ===")
    fec_packets = receive_fec_data(timeout_after_first=timeout_after_first)

    rlc_sdus = []  # Collect valid RLC payloads

    for idx, pkt in enumerate(fec_packets, 1):
        print(f"\n--- [Packet {idx}/{len(fec_packets)}] ---")

        # Step 1: Convert to bits (remove 2 tail bits)
        fec_bits = np.unpackbits(np.frombuffer(pkt, dtype=np.uint8))[:-2]
        ...
        # Step 4: CRC Check
        mac_payload = mac_strip_crc(mac_bytes)
        if mac_payload is None:
            continue

        # Step 5: RLC reassembly prep: keep RLC PDUs
        rlc_sdus.append(mac_payload)

    if not rlc_sdus:
        print("[RLC] No valid RLC PDUs received.")
        return

    # Reassemble at RLC (across multiple PDUs)
    full_rlc_payload = b''
    for sdu in rlc_sdus:
        part = rlc_reassemble(sdu)
        if part:
            full_rlc_payload += part

    if not full_rlc_payload:
        print("[RLC] No valid payloads assembled.")
        return

    # PDCP Decryption
    pdcp_data = pdcp_decrypt(full_rlc_payload)
    if pdcp_data is None:
        return
